...
public class LineUtils {
  private static double AngleBetweenLines(Line ln1, Line ln2) {
    double angle1 = Math.toDegrees(Math.atan2(ln1.getY2() - ln1.getY1(), ln1.getX1() - ln1.getX2()));
    double angle2 = Math.toDegrees(Math.atan2(ln2.getY2() - ln2.getY1(), ln2.getX1() - ln2.getX2()));
  ...
  public static List<Line> GetLargestAngleLines(List<Line> lines) {
    ...
    double maximum = Math.max(ang01, Math.max(ang12, Math.max(ang23, Math.max(ang02, Math.max(ang03, ang13)))));
    if (maximum == ang01) {
  ...
  public static List<Point> GetLargestAnglePoints(Point main, List<Point> allPoints) {
    ...
    for (int i = 0; i < allPoints.size(); i+=2) {
      if (LineUtils.AngleBetweenPoints(main, allPoints.get(i), allPoints.get(i+1)) > biggestAngle) {
